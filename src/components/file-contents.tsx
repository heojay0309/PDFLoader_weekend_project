        ...
        <div className="flex h-full flex-1 flex-col gap-4 overflow-y-auto">
          {uploadedFiles.map((file) => {
            ...
            return (
              <div
                key={file.id}
                className="flex min-h-[64px] w-full flex-col items-center justify-between gap-2 rounded-2xl border border-white/10 bg-[#191919] p-4 text-white hover:border-primary/60 hover:bg-[#191919] active:border-primary/80 active:bg-[#191919]"
              >
                {file.isLoading ? (
                  <h4>Loading...</h4>
                ) : (
